...
Os identificadores de usuário e grupo (UIDs/GIDs) são as referências básicas numeradas das
contas.
...
Via de regra, qualquer usuário que não seja o root é considerado um usuário regular.
...
As contas de sistema têm:
* UIDs menores que 100 ou entre 500 e 1000 (de dois a três dígitos)
* Não têm diretório inicial dedicado e, se tiverem, ele não está em /home
* Não têm shell de login válido
...
Os arquivos de controle de acesso ficam no diretório **/etc**, onde costumam ser armazenados os arquivos de configuração do sistema. Por padrão,
esse diretório pode ser lido por todos os usuários do sistema, mas modificado apenas pelo root.
...
* Armazena o UID, GID, shell, diretório inicial etc.
...
**GECOS**
Uma lista em formato CSV de informações de usuários, incluindo nome, localização e número de
...
* Hash de senha que permite que usuários se tornem membros temporários do grupo
...
selecionados de autenticação podem ler e que o superusuário pode modificar por meio de outros
comandos.
