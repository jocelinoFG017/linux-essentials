...
Se excluirmos o arquivo para o qual o link aponta (chamado destino), o link ainda existirá, mas deixará de funcionar, pois passará a apontar para um destino inexistente.

Utilizamos o comando `ln` com o parâmetro `-s` para criar links simbólicos.

Se o arquivo-alvo (target) for movido, o link quebrará.
...
Um link físico é como um segundo nome para o arquivo original. Links físicos não são duplicatas, mas entradas adicionais no sistema de arquivos que apontam para o mesmo local (inode) no disco.
